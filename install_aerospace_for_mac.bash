...
directory_exists_with_spaces "${USER_HOME}"
 ...
 #--------\/\/\/\/-- 2tasks_templates_sudo/aerospace_for_mac …install_aerospace_for_mac.bash” -- Custom code -\/\/\/\/-------
...
_package_list_installer() {
  ...
  trap 'echo -e "${RED}" && echo "ERROR failed $0:$LINENO _package_list_installer aerospace" && echo -e "${RESET}" && return 0' ERR
...
_add_variables_to_bashrc_zshrc(){
  [[ ! -f "${USER_HOME}/.aerospace.toml" ]] && cp /Applications/AeroSpace.app/Contents/Resources/default-config.toml "${USER_HOME}/.aerospace.toml"
  local aerospace_SH_CONTENT='

# AEROSPACE
if [[ -e "'${USER_HOME}'/.aerospace" ]] ; then
{
  export AEROSPACE_ROOT="'${USER_HOME}'/.aerospace"
  # export PATH="'${USER_HOME}'/bin:${PATH}"
  # export PATH="'${USER_HOME}'/.aerospace/bin:${PATH}"
}
fi
'
  trap 'echo -e "${RED}" && echo "ERROR failed $0:$LINENO _add_variables_to_bashrc_zshrc aerospace" && echo -e "${RESET}" && return 0' ERR
  Checking "${aerospace_SH_CONTENT}"
  ...
  while read INITFILE; do
  {
    ...
    _if_not_contains "${USER_HOME}/${INITFILE}"  "# AEROSPACE" ||  echo "${aerospace_SH_CONTENT}" >> "${USER_HOME}/${INITFILE}"
    _if_not_contains "${USER_HOME}/${INITFILE}"  "AEROSPACE_ROOT" ||  echo "${aerospace_SH_CONTENT}" >> "${USER_HOME}/${INITFILE}"
  }
  done <<< "${INITFILES}"
  # type aerospace
  # Checking "export PATH=\"${USER_HOME}/.aerospace/bin:${PATH}\" "
  # export PATH="${USER_HOME}/.aerospace/bin:${PATH}"
  chown -R "${SUDO_USER}" "${USER_HOME}/.aerospace"
  chown -R "${SUDO_USER}" "${USER_HOME}/.aerospace.toml" 
  # cd "${USER_HOME}/.aerospace/bin"
...
_debian_flavor_install() {
  ...
  su - "${SUDO_USER}" -c "bash -c 'brew install --cask nikitabobko/tap/aerospace'"
  local MSG=$(_add_variables_to_bashrc_zshrc)
  echo "${MSG}"
  ensure aerospace or "Canceling until aerospace did not install"
  echo "Documentation: HREF: https://nikitabobko.github.io/AeroSpace/guide"
...
_uaerospacetu__32() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  _debian_flavor_install
} # end _uaerospacetu__32

_uaerospacetu__64() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  _debian_flavor_install
} # end _uaerospacetu__64

_uaerospacetu__aarch64() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  _uaerospacetu_22__aarch64
} # end _uaerospacetu__aarch64

_uaerospacetu_22__aarch64() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  trap 'echo -e "${RED}" && echo "ERROR err:$_err failed $0:$LINENO _debian_flavor_install aerospace" && echo -e "${RESET}" && return 0' ERR
  ensure brew or "Canceling until brew is installed"
  su - "${SUDO_USER}" -c "bash -c 'brew install --cask nikitabobko/tap/aerospace'"
  local MSG=$(_add_variables_to_bashrc_zshrc)
  echo "${MSG}"
  ...
  echo "Documentation: HREF: https://nikitabobko.github.io/AeroSpace/guide"
  # su - "${SUDO_USER}" -c 'aerospace install -l'
  # su - "${SUDO_USER}" -c 'aerospace install 3.1.4'
  # ensure ruby or "Canceling until ruby is not working"
  # su - "${SUDO_USER}" -c 'ruby -v'
} # end _uaerospacetu_22__aarch64
...
_darwin__64() {
  ...
  # su - "${SUDO_USER}" -c "${USER_HOME}/.aerospace/bin/aerospace install -l"
  # su - "${SUDO_USER}" -c "${USER_HOME}/.aerospace/bin/aerospace install 2.6.5"
  # ensure ruby or "Canceling until ruby is not working"
  # su - "${SUDO_USER}" -c 'ruby -v'
} # end _darwin__64
