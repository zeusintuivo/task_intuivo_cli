...
directory_exists_with_spaces "${USER_HOME}"
 ...
 #--------\/\/\/\/-- 2tasks_templates_sudo/test …install_test.bash” -- Custom code -\/\/\/\/-------
...
_debian_flavor_install() {
  ...
  dpkg -i slack-desktop-4.27.154-amd64.deb
  snap install slack
  echo to remove snap remove slack
  # flatpak install flathub com.slack.Slack
  exit 0
  # echo to remove flatpak uninstall flathub com.slack.Slack
  if (
...
_redhat_flavor_install() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  snap refresh
  echo This was old command snap install slack-classic
  anounce_command snap install slack
  echo to remove snap remove slack
  # anounce_command flatpak install flathub com.slack.Slack
  # echo to remove flatpak uninstall flathub com.slack.Slack
...
_fedora_37__64() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  ( it_does_not_exist "holafile" ) &&  echo "Correct behavior it does not exists holafile"
	if ( it_does_not_exist "install_code.bash" ) ; then
  {
    echo "Correct behavior it does not exists holafile"
  }
  else
  {
    echo "File does exist install_code.bash"
  }
  fi
  #_fedora__64 "${*-}"
} # end _fedora_37__64
