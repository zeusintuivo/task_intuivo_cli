...
#
 ...
 #--------\/\/\/\/-- 2tasks_templates_sudo/php …install_php.bash” -- Custom code -\/\/\/\/-------
...
_redhat_flavor_install() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  echo "_REF: https://utho.com/docs/tutorial/how-to-install-php-8-on-fedora-38/"
  echo "fedora_37 $(rpm -E %fedora)"
...
_fedora_43__64() {
  ...
  if [[ -n "$PHP_STREAM" ]]; then
    log "Switching PHP module stream to ${PHP_STREAM}"
    as_root "$DNF" -y module reset php
    as_root "$DNF" -y module enable "php:${PHP_STREAM}"
  fi
  ...
  log "Enabling nginx, mariadb, and php-fpm services"
  enable_and_start_service php-fpm.service
...
restart_service_if_present() {
  ...
  if have_cmd systemctl && systemctl list-unit-files "$service" >/dev/null 2>&1; then
