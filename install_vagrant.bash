...
directory_exists_with_spaces "${USER_HOME}"
 ...
 #--------\/\/\/\/-- 2tasks_templates_sudo/vagrant …install_vagrant.bash” -- Custom code -\/\/\/\/-------
...
_package_list_installer() {
  ...
  trap 'echo -e "${RED}" && echo "ERROR failed $0:$LINENO _package_list_installer vagrant" && echo -e "${RESET}" && return 0' ERR
...
_add_variables_to_bashrc_zshrc(){
  [[ ! -d "${USER_HOME}/.vagrant" ]] && mkdir -p "${USER_HOME}/.vagrant"
  local vagrant_SH_CONTENT='

# VAGRANT
if [[ -e "'${USER_HOME}'/.vagrant" ]] ; then
{
  export VAGRANT_ROOT="'${USER_HOME}'/.vagrant"
  # export PATH="'${USER_HOME}'/bin:${PATH}"
  # export PATH="'${USER_HOME}'/.vagrant/bin:${PATH}"
}
fi
'
  trap 'echo -e "${RED}" && echo "ERROR failed $0:$LINENO _add_variables_to_bashrc_zshrc vagrant" && echo -e "${RESET}" && return 0' ERR
  Checking "${vagrant_SH_CONTENT}"
  ...
  while read INITFILE; do
  {
    ...
    _if_not_contains "${USER_HOME}/${INITFILE}"  "# VAGRANT" ||  echo "${vagrant_SH_CONTENT}" >> "${USER_HOME}/${INITFILE}"
    _if_not_contains "${USER_HOME}/${INITFILE}"  "VAGRANT_ROOT" ||  echo "${vagrant_SH_CONTENT}" >> "${USER_HOME}/${INITFILE}"
  }
  done <<< "${INITFILES}"
  # type vagrant
  # Checking "export PATH=\"${USER_HOME}/.vagrant/bin:${PATH}\" "
  # export PATH="${USER_HOME}/.vagrant/bin:${PATH}"
  chown -R "${SUDO_USER}" "${USER_HOME}/.vagrant"
  # cd "${USER_HOME}/.vagrant/bin"
...
_debian_flavor_install() {
  ...
  su - "${SUDO_USER}" -c "bash -c 'brew tap hashicorp/tap'"
  su - "${SUDO_USER}" -c "bash -c 'brew install hashicorp/tap/hashicorp-vagrant'"
  local MSG=$(_add_variables_to_bashrc_zshrc)
  echo "${MSG}"
  ensure vagrant or "Canceling until vagrant did not install"
...
_uvagranttu__aarch64() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  _uvagranttu_22__aarch64
} # end _uvagranttu__aarch64

_uvagranttu_22__aarch64() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  trap 'echo -e "${RED}" && echo "ERROR err:$_err failed $0:$LINENO _debian_flavor_install vagrant" && echo -e "${RESET}" && return 0' ERR
  ensure brew or "Canceling until brew is installed"
  su - "${SUDO_USER}" -c "bash -c 'brew tap hashicorp/tap'"
  su - "${SUDO_USER}" -c "bash -c 'brew install hashicorp/tap/hashicorp-vagrant'"
  local MSG=$(_add_variables_to_bashrc_zshrc)
  ...
  # su - "${SUDO_USER}" -c 'vagrant install -l'
  # su - "${SUDO_USER}" -c 'vagrant install 3.1.4'
  # ensure ruby or "Canceling until ruby is not working"
  # su - "${SUDO_USER}" -c 'ruby -v'
} # end _uvagranttu_22__aarch64
...
_darwin__64() {
  ...
  # su - "${SUDO_USER}" -c "${USER_HOME}/.vagrant/bin/vagrant install -l"
  # su - "${SUDO_USER}" -c "${USER_HOME}/.vagrant/bin/vagrant install 2.6.5"
  # ensure ruby or "Canceling until ruby is not working"
  # su - "${SUDO_USER}" -c 'ruby -v'
} # end _darwin__64
