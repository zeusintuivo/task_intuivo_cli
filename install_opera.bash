...
directory_exists_with_spaces "${USER_HOME}"
 ...
 #--------\/\/\/\/-- 2tasks_templates_sudo/opera …install_opera.bash” -- Custom code -\/\/\/\/-------
...
_redhat_flavor_install() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  Comment htps://www.linuxcapable.com/install-opera-on-fedora-linux/
	dnf upgrade --refresh -y --allowerasing
	rpm --import https://rpm.opera.com/rpmrepo.key
	dnf config-manager --add-repo https://rpm.opera.com/rpm
	# dnf builddep opera-stable  -y --allowerasing
	dnf install opera-stable -y --allowerasing
	# dnf install opera-beta -y --allowerasing
	# dnf install opera-developer -y --allowerasing
