...
_redhat_flavor_install() {
  ...
  dnf build-dep openssl-devel -vy --allowerasing
  dnf build-dep openssl       -vy --allowerasing
  dnf install   openssl-devel -vy --allowerasing
  dnf install   openssl       -vy --allowerasing
  # Batch Fedora 37
  local package packages="
    libyaml
    libyaml-devel
  ...
  su - "${SUDO_USER}" -c 'rbenv install 3.3.5'
  su - "${SUDO_USER}" -c 'rbenv global 2.6.5'
  su - "${SUDO_USER}" -c 'rbenv rehash'
  ensure ruby or "Canceling until ruby is not working"
  su - "${SUDO_USER}" -c 'ruby -v'
