...
_find_location_brew_files() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  local _sub_directory=${1:-}
  enforce_parameter_with_value           1        _sub_directory     "${_sub_directory:-}"     "var/postgresql@12 | "

  Checking "location of brew ${_sub_directory} "
	local _result=""
  local _brew_variations="
		   /opt/homebrew/${_sub_directory}
		   /home/linuxbrew/.linuxbrew/${_sub_directory}
		   /usr/local/${_sub_directory}
	"
	local one=""
	while read -r one ; do 
	{
  ...
  done <<< "${_brew_variations}"

	[[ -z "${_result}" ]] && return 1
	echo -n "${_result}"
	return 0	
} # end _find_location_brew_files

_try_more_times_find() {
  trap  '_trap_on_error $0 "${?}" LINENO BASH_LINENO FUNCNAME BASH_COMMAND $FUNCNAME $BASH_LINENO $LINENO   $BASH_COMMAND'  ERR
  local _possibles=${*:-}
  enforce_parameter_with_value           1        _possibles     "${_possibles:-}"     "
          var/postgresql@12
          var/postgres
          var/postgresql
   "
  ...
  local _result=""
	...
	{
	  [[ -z "${one}" ]] && continue
		_result="$(_find_location_brew_files "${one}" )"
	  _err=$?
    if [ $_err -eq 0 ] ; then 
  	{
			break
    ...
    echo "${_result}"
		warning  "try ${counter} one Could not find/determine location of ${one}"
		counter=$(( counter + 1 ))
	}
  done <<< "${_possibles}"
...
# EOF
    ...
    if (su - "${SUDO_USER}" -c "brew services" 2>&1 | grep "${__pg}" >/dev/null 2>&1; )  ; then
    {
      ...
      if [ $_err -gt 0 ] ; then # no
      {
        ...
        su - "${SUDO_USER}" -c "ARCHFLAGS='-arch $(uname -m)' brew reinstall shared-mime-info"
        su - "${SUDO_USER}" -c "ARCHFLAGS='-arch $(uname -m)' brew reinstall libpq"
        su - "${SUDO_USER}" -c "ARCHFLAGS='-arch $(uname -m)' brew reinstall ruby-build"
        su - "${SUDO_USER}" -c "ARCHFLAGS='-arch $(uname -m)' brew reinstall redis"
      }
		  else
			{
        ...
        else
        {
          ...
          ( su - "${SUDO_USER}" -c "brew remove ${__pg}" )
          if [ "$(pgrep postgres 2>&1  | wc -l | xargs )" -gt 0 ]  ; then
          {
            killall postgres
			    ...
			    Installing "${__pg}"
          su - "${SUDO_USER}" -c "ARCHFLAGS='-arch $(uname -m)' brew install shared-mime-info"
          su - "${SUDO_USER}" -c "ARCHFLAGS='-arch $(uname -m)' brew install libpq"
          su - "${SUDO_USER}" -c "ARCHFLAGS='-arch $(uname -m)' brew install ruby-build"
          su - "${SUDO_USER}" -c "ARCHFLAGS='-arch $(uname -m)' brew install redis"
...
_brew_postgres_redis_restart() {
    ...
    if su - "${SUDO_USER}" -c "brew services restart  postgresql@12 " ; then
		{
			warning failed brew services restart  postgresql@12 
		}
		fi
		Message "brew services restart  redis"
    # anounce_command brew services restart  redis
		# anounce_command su - "${SUDO_USER}" -c "brew services restart  redis "
		if su - "${SUDO_USER}" -c "brew services restart  redis " ; then 
		{
			warning failed brew services restart  redis
		}
		fi
		Message "brew services"
    # anounce_command brew services
    # anounce_command su - "${SUDO_USER}" -c "brew services "
    if su - "${SUDO_USER}" -c "brew services " ; then 
		{
			warning brew services
		}
		fi
...
")"
  ...
  bundle config build.pg --with-pg-config=${_libpg_bin}/pg_config
