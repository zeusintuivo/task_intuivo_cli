...
directory_exists_with_spaces "${USER_HOME}"
 ...
 #--------\/\/\/\/-- 2tasks_templates_sudo/skype …install_skype.bash” -- Custom code -\/\/\/\/-------
...
_redhat_flavor_install() {
  ...
  local _parameters="${*-}"
  local -i _err=0
  _remove_package_if_it_exists skypeforlinux "old skype version of skype stopped working on 2024"
	_err=$?
  if [ ${_err} -gt 0 ] ; then
  {
  ...
  if [ -e /etc/yum.repos.d/skype-stable.repo ] ; then
  {
	  rm -rf /etc/yum.repos.d/skype-stable.repo
	}
	fi 
...
gpgkey=file:///usr/share/distribution-gpg-keys/skype/SKYPE-GPG-KEY
enabled=0
" > /etc/yum.repos.d/microsoft-skype.repo
  yes | dnf install distribution-gpg-keys -y
  yes | rpm --import /usr/share/distribution-gpg-keys/skype/SKYPE-GPG-KEY
  yes |	dnf install skypeforlinux -y
	rpm -qa | grep skypeforlinux
